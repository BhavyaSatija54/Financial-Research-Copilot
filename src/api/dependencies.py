"""FastAPI dependency injection: auth, shared service singletons."""
...
def get_metadata_db() -> MetadataDB:
    assert _metadata_db is not None, "MetadataDB not initialised"
    return _metadata_db


async def initialise_services(settings: Settings) -> None:
    global _faiss_store, _bm25_retriever, _embedder
    global _cache_client, _research_agent, _ingestion_pipeline, _metadata_db
    ...
    if settings.cache_enabled:
        _cache_client = CacheClient(url=settings.redis_url, ttl=settings.cache_ttl_seconds, enabled=True)
