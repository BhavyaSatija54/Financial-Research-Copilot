...
class Settings(BaseSettings):
    ...
    metadata_db_path: Path = Path("data/processed/metadata.db")
    ...
    sec_request_delay_seconds: float = 0.11

    # ── FRED Macro ───────────────────────────────────────────────────
    fred_api_key: SecretStr | None = Field(None, description="FRED API key for macro data")
    ...
    @property
    def fred_api_key_str(self) -> str | None:
        return self.fred_api_key.get_secret_value() if self.fred_api_key else None

    @field_validator("faiss_index_path", "bm25_index_path", "metadata_db_path", mode="before")
    @classmethod
